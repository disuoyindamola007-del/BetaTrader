...
export default function NewsScreen() {
  ...
  return (
    <div className="px-4 pt-4 pb-6 animate-fade-in">
      ...
      {isLoading && <div className="glass-card p-8 text-center"><RefreshCw size={20} className="mx-auto text-emerald-400 animate-spin" /><p className="text-sm text-slate-500 mt-2">Loading live news...</p></div>}
      ...
      {!isLoading && !error && <div className="flex flex-col gap-3">{news.map(article => <button key={article.id} onClick={() => navigateToNews(article)} className="glass-card-hover p-4 text-left"><p className="text-xs text-slate-500 mb-2">{article.source}</p><p className="text-sm font-semibold leading-relaxed mb-2">{article.headline}</p><p className="text-xs text-slate-500 line-clamp-2">{article.summary}</p></button>)}</div>}
    </div>
  );
}
