...
import { Eye, EyeOff, Loader2, Mail, Lock, UserPlus, LogIn } from 'lucide-react';
...
export default function AuthScreen() {
  ...
  const submit = async (event) => {
    ...
    try {
      const result = mode === 'sign-in'
        ...
        : await supabase.auth.signUp({ email: cleanEmail, password, options: { data: { display_name: displayName.trim() || undefined } } });
  ...
  return (
    <main className="min-h-screen theme-bg-primary flex items-center justify-center px-5 py-8">
      <div className="w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-emerald-500/20">B</div>
          <h1 className="text-2xl font-extrabold theme-text-primary">Welcome to BetaTrader</h1>
          <p className="text-sm theme-text-secondary mt-2">Your trading companion, securely synced.</p>
        </div>
        ...
        <p className="text-center text-[11px] theme-text-muted mt-4">Your data belongs to your account and is protected by row-level security.</p>
      </div>
    </main>
  );
}
